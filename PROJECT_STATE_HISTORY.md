...
- 本轮已停止旧同步任务并清理旧导入内容；新的公开 API 同步任务直接写入项目内 `service/data/dutongjian.db`、`service/data/tongjian-cache/` 和 `service/data/tongjian-progress.json`，不会使用临时目录作为内容源。
...
- 旧导入内容的可恢复快照和本轮新数据均保存在项目的 `service/data/` 下；原始 API JSON 缓存位于项目的 `service/data/tongjian-cache/`，完成后由 `service/app/export_android.py` 导出到 Android assets。
- 新增的 `--reset` 只用于首次清理旧通鉴内容、缓存和 checkpoint；开机服务不带 `--reset`，只从项目内 checkpoint 续跑。
...
- 旧 Release 的 `4552` 条正文属于历史产物；本轮新抓取完成后会从项目内数据库重新校验并导出新的 Android assets，再重新构建 APK。

## 当前同步与内容路径（2026-08-03）

- 新同步器使用项目内路径：数据库 `service/data/dutongjian.db`、原始缓存 `service/data/tongjian-cache/`、断点 `service/data/tongjian-progress.json`。
- `android/app/src/main/assets/` 是最终离线内容的项目内落点；未完成同步前不生成阶段性古籍资产。
- `/tmp` 不作为新抓取内容、数据库、缓存或 checkpoint 的存储位置；项目内的 `service/data/resync-archive-20260803/` 仅保存清理前旧内容的可恢复审计快照。
- 已安装 `deploy/readgb-crawler.service` 对应的 systemd 开机续跑配置，宿主机重启后沿用上述项目路径继续同步。
