...
这能随真实资产规模扩展；当前数据库和 APK assets 已达到全量 `30,989` 条正文、`294` 卷、`1,405` 年，图表数量和关系覆盖仍需设备级性能与准确性回归。Android 设备级性能回归不纳入当前交付范围。
...
- [x] 离线资产：已严格通过数据门槛后生成正文、目录、百科三个 Android assets，并从 Debug/Inspection/unsigned Release APK 内容核对数量和 SHA-256；APK 仅上传 GitHub Release。✅
...
- [ ] AI 端到端：用本地或用户提供的 OpenAI-compatible 服务验证请求、错误、超时和结果展示；结果保存/删除/重开、语法原文定位高亮、反事实上下文边界和人物多轮会话已实现，仍需真实模型服务验证事实质量。
