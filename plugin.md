...
- [x] Edge-TTS 已修复协议层无声问题：移除 Edge 会以 WebSocket `1007` 拒绝的 `<s>` 标签，并补齐空音频、播放器和握手错误处理；真实设备音频仍待验收。✅
- [x] 正文详情页按原书 `sort_order` 提供相邻篇目和篇目选择器，避免读完一章后返回目录寻找下一篇。✅
...
- [ ] OpenAI 兼容接口/本机模型配置、API Key 本地保存和 AI 任务入口已接入；角色追问、史料边界和结果清理已实现，但真实模型请求/错误/超时仍需可用服务端到端验收。
- [x] 古文语法拆解支持 Markdown 表格解析和结构化结果卡片，格式异常时回退原文。✅
- [x] 古文语法拆解支持可选“原文定位”列；客户端对命中的连续原文片段高亮，旧模型输出仍回退显示。✅
- [x] AI 结果已实现 Room 保存、删除和重开；覆盖反事实/角色等任务的结果记录，迁移和 JVM 测试已通过。✅
- [x] 反事实推演已有独立模板、史料数据边界和事实/推断/假设/影响分栏要求；真实模型端到端调用尚未验收。✅
- [x] 历史人物角色对话支持选择两位人物、同一史料边界内的多轮追问和用户插话，并将会话转录更新到 Room；事实质量和离线/联网端到端验收仍待真实模型服务。✅
- [ ] 重点战役沙盘的地图、兵力、粮道和关键决策数据仍未公开结构化；当前已实现可追溯的史料证据视图，缺失字段明确标注，不宣称战役推演已完成。
