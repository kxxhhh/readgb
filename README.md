...
- 详情页支持原文/译文并读、原文/白话单独模式、字号、繁简/异体字展示、复制、分享、划线笔记、历史上下文和按原书篇次的相邻篇目导航。
- 系统 TTS 控制器、句子状态、自动滚动和睡眠计时器代码；Edge-TTS 是可选网络引擎，已修复协议层无声问题，真实音频仍需设备验证。
- 历史角色对话可选择两位人物；生成后用户可插话，两位人物会在同一史料边界内分别回应。史料沙盘展示可追溯证据和缺失数据边界，不虚构兵力、粮道或路线。
